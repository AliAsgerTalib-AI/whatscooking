import type { ProFields } from "../types/recipe";

interface ProFieldsPanelProps {
  proFields: ProFields;
  onChange:  (fields: ProFields) => void;
}
...
export function ProFieldsPanel({ proFields, onChange }: ProFieldsPanelProps) {
  const f   = proFields;
  const set = (key: keyof ProFields, val: string) => onChange({ ...f, [key]: val });
